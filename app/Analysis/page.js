...
import articles from "@/components/AnalysisArticles";
  ...
  return (
      <div className="py-10 px-10 flex justify-center">
          <div className="flex flex-col items-center gap-7">
              <div className="font-bold  md:text-5xl text-4xl">Analysis</div>
                    ...
                    <div className="flex flex-col gap-7  w-full items-center">
                        {Object.values(articles).map(function(article) {
                            if (article.id % 3 === 1 && article.title !== "") {
                                return <Link key={article.id} href={`/Analysis/${article.id}`} className="h-min-50 md:w-100 w-90 bg-white hover:cursor-pointer rounded-xl shadow-2xl p-4">
